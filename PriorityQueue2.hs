...
keycmp :: (Ord k) => ( k -> k -> Ordering) -> ( (k,a) -> (k,a) -> Ordering)
...
-- | /O(1)/ Returns a priority queue with x as the only element
...
-- | Helper function for working with key-value pairs in a priority
-- queue.  This inserts a key-value pair into a priority queue, using
-- the Ord instance of the key to determine ordering.
insertKV :: (Ord k) => (k,v) -> PriorityQueue (k,v) -> PriorityQueue (k,v)	      
insertKV (k,v) pq = insertBy stdcmp (k,v) pq

-- | Helper function for working with key-value pairs in a priority
-- queue.  This deletes the minimum key-value pair from a priority
-- queue, using the Ord instance of the key to determine ordering.
...
fromListKV = foldr insertKV empty
